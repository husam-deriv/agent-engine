...
def generate_mermaid_flowchart(chart_description: str) -> str:
    """Generate MermaidJS flowchart syntax from a textual description using OpenAI.

    Args:
        chart_description: A textual description of the chart/visual to be generated.
                           Example: "A flowchart with a start node leading to a process node, which then leads to an end node."

    Returns:
        A string containing the MermaidJS syntax for the flowchart, enclosed in ```mermaid ... ```.
        Returns an error message string if the API call fails or the response is unexpected.
    """
    try:
