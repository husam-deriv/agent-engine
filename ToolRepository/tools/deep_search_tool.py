...
class DeepResearch:
    ...
    def __init__(self, disable_cache=False):
        ...
        self.tavily_api_key = os.environ.get("TAVILI_API_KEY")
...
"""
    ...
    def perform_research(self, query: str, num_results: int = 15) -> Dict:
        """
...
def deep_research(query: str, disable_cache: bool = False, num_results: int = 15) -> Dict:
    """
    Perform deep research on a topic, returning a comprehensive analysis.
    
    This function conducts web searches using available search providers
    (Tavily and/or Google), analyzes the results using LLMs (GPT-4.1 
    or Gemini 2.5 Flash), and provides a structured report with 
    chain-of-thought reasoning, sources, and reliability assessments.
    
    Args:
        query (str): The research question or topic
        disable_cache (bool): Whether to disable caching (default: False)
        num_results (int): Maximum number of search results to request (default: 15)
        
    Returns:
        Dict: A comprehensive research report with the following structure:
            {
                "success": bool,
                "query": str,
                ...
                "analysis": {
                    ...
                    "insights": List[Dict],  # Subtopics with confidence scores
                    "sources": List[Dict],  # Sources with reliability ratings
                    "research_gaps": List[str],
                    "conflicting_info": List[str]
                },
                "visual_data": Dict,  # Visualization-friendly data structures
                "model_used": str,  # Which model performed the analysis
                "sources_count": int,
                "search_providers_used": List[Dict]
            }
    ...
    # Perform research
    return research_tool.perform_research(query, num_results=num_results)
